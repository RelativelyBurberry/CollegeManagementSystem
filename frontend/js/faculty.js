    ...
    async function loadFacultyProfile() {
        ...
        if (welcome) {
            welcome.innerText = `Welcome back, ${faculty.name}`;
        }

        // 🔥 INITIALS FIX
        const initialsEl = document.querySelector(".profile-img");
        if (initialsEl && faculty.name) {
        ...
        // 🔥 PAGE TITLE FIX
        document.title = `${faculty.name} | Faculty Portal`;

        // 🔥 PROFESSOR TITLE FROM DB
        const roleEl = document.querySelector(".profile-role");
        if (roleEl && faculty.department) {
            roleEl.innerText = `Professor of ${faculty.department.name}`;
        }
        ...
        if (welcomeText && data) {
            if (data.next_class) {
                welcomeText.textContent =
                    `You have ${data.classes_today} classes scheduled for today. ` +
                    `Your next lecture is ${data.next_class.course} at ` +
                    `${data.next_class.time} in ${data.next_class.room}.`;
            } else {
                welcomeText.textContent =
                    `You have ${data.classes_today} classes scheduled for today. ` +
                    `No more classes remaining today.`;
            }
        }
    ...
    async function loadFacultyCourses() {
        ...
        /* ===============================
        2️⃣ POPULATE DROPDOWNS (existing)
        ================================ */
        const classSelect = document.getElementById("classSelect");
        const resultClassSelect = document.getElementById("resultClassSelect");

        if (classSelect) classSelect.innerHTML = "";
        if (resultClassSelect) resultClassSelect.innerHTML = "";

        courses.forEach(c => {
            if (classSelect) {
                classSelect.add(new Option(`${c.course_code}: ${c.course_name}`, c.id));
            }
            if (resultClassSelect) {
                resultClassSelect.add(new Option(`${c.course_code}: ${c.course_name}`, c.id));
            }
        });

        /* ===============================
        3️⃣ SAFE AUTO-LOAD ATTENDANCE
        ================================ */
        if (courses.length > 0 && document.getElementById("classSelect")) {
    ...
    document.getElementById("coursesCard")
        .addEventListener("click", loadCoursesModal);
    ...
    async function loadPapersModal() {
        ...
        data.forEach(p => {
            ...
            modalBody.innerHTML += `
                <div style="background: rgba(0,0,0,0.02); padding: 12px; border-radius: 8px; margin-bottom: 8px;">
                    <div style="display: flex; justify-content: space-between;">
                        <strong>${p.course}: ${p.title}</strong>
                        <span>${p.pending} papers</span>
                    </div>
                </div>
            `;
        });
        ...
        modalBody.insertAdjacentHTML(
            "afterbegin",
            `<p>You have <strong>${total}</strong> papers waiting to be graded:</p>`
        );
    }
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        /* ===============================
        STAT CARD → STUDENTS MODAL
        ================================ */
        const studentsCard = document.getElementById("studentsCard");
        if (studentsCard) {
            studentsCard.addEventListener("click", async () => {
                await loadStudentsModal();
                openModal("studentsModal");
            });
        }

        const papersCard = document.getElementById("papersCard");
        if (papersCard) {
            papersCard.addEventListener("click", async () => {
                await loadPapersModal();
                openModal("papersModal");
            });
        }
    ...
    window.saveAllGradeChanges = saveAllGradeChanges;
